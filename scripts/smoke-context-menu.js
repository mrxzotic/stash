...
assert.equal(manifest.commands["save-to-tuckio"].suggested_key.default, "Alt+Shift+S");
assert.equal(manifest.commands["toggle-tuckio"].suggested_key.default, "Alt+Shift+O");
assert.equal(manifest.commands["save-to-tuckio"].description, "Save current page to Tuckio");
assert.equal(manifest.commands["toggle-tuckio"].description, "Open or close Tuckio");
...
assert.match(backgroundSource, /COMMAND_SAVE_CURRENT = "save-to-tuckio"/, "Save shortcut command id should match manifest");
assert.match(backgroundSource, /COMMAND_TOGGLE_PANEL = "toggle-tuckio"/, "Toggle shortcut command id should match manifest");
...
assert.match(backgroundSource, /const MENU_ROOT_ID = "tuckio-save-root"/);
assert.match(backgroundSource, /title: "Save to Tuckio"/);
...
assert.match(backgroundSource, /Tuckio could not create context menu/, "Unexpected menu creation failures should be logged");
