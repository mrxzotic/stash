...
- Search and More options are bare icon buttons: clickable area is fine, but no visible border, fill, or shadow.
...
- More options opens as a compact overflow menu with Dark mode, Language, and About. It is not a grouped settings surface.
- Overflow menu spacing follows the 8px grid: row gaps, dropdown offsets, menu padding, and option radii must stay visibly separated, not stacked edge-to-edge.
- Categories are managed from the filter menu, not More options.
...
- Summary currency is selected from the topbar total pill and is the only summary currency shown.
...
- Typography uses fixed project tokens, not ad hoc pixel sizes:
  ...
  - `--text-body: 13px` for category pills, overflow rows, dropdown items, empty-state body, and standard metadata.
...
- Use restrained black/white neutrals with translucent panel backgrounds. Default background is Warm; More options toggles Graphite. Keep the main surface clean; subtle mesh/noise texture is allowed only inside the top and bottom transparent fade layers.
