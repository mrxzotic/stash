...
- Every completed code/content change must end with a local git commit and immediate push to GitHub. Check `git status` first, stage only intended project files, run the cheapest relevant checks, commit with a concise imperative message, then push the current tracking branch before reporting done.
